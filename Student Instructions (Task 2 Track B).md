...
You will produce a **redesign specification** for a substantially improved version of Scotiabank's AI Ethics Assistant. Your task is not to write code. It is to commit, in writing, to a vision of what the next generation of this governance tool should be — and to defend the trade-offs your vision involves.

This task builds directly on the critique you produced in Task 1. Track B is the constructive counterpart: having identified what the current tool gets wrong, you must now decide what would be better, and why.
...
| File | What It Is |
|------|-----------|
| `ethics_assessment.html` | The working prototype, available to inspect. Open it in a browser. You do not need to modify it. |
| This file | Your instructions, required components, and evaluation criteria. |

A description of the prototype's structure (dimensions, scoring logic, radar chart output, action-guidance mechanism) is also embedded in the case (Appendices B and C). Use the live tool to confirm any detail the case leaves ambiguous.
...
### 2. How it would handle the self-reporting problem

The AMCA case shows that submitters can characterise their own systems inaccurately — sometimes inadvertently, sometimes self-servingly — and the current tool depends on the AI & Ethics Governance team's review to catch this. Propose a concrete mechanism (or combination of mechanisms) that reduces the tool's structural reliance on self-reporting. Examples to consider, not be limited by: required documentation links, automated inconsistency detection, evidence-based questions ("attach the model card"), independent verification triggers.

### 3. How it would communicate risk honestly without losing usability

The current tool produces a single score, displayed on a radar chart, with auto-generated action guidance. The score is technically accurate yet — as the AMCA result demonstrates — can mask material gaps. Propose how the revised tool should communicate risk to a non-specialist submitter. What does a score do well? What does it obscure? What would a more honest output look like, and what does it cost in usability?
